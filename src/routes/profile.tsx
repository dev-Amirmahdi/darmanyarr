...
function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (p)
      setCount({
        apts: appointmentsRepo.byPatient(p.email).length,
        notif: notificationsRepo.list().filter((n) => !n.read).length,
      });
  }, []);
    ...
    return (
      <div className="mx-auto max-w-md px-5 py-16 text-center">
        ...
        <p className="text-muted-foreground mb-6">
          برای دسترسی به پروفایل و نوبت‌ها ابتدا وارد شوید.
        </p>
        <Link
          to="/auth"
    ...
    <div className="mx-auto max-w-2xl px-5 py-8">
      <div className="card-elevated p-6 flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-black">{patient.username}</h1>
      ...
      <div className="mt-6 card-elevated p-2">
        {items.map((it, i) => {
          ...
          return it.to ? (
            ...
            <button key={i} onClick={it.onClick} className="w-full text-right">
              {inner}
            </button>
          );
        })}
      ...
      <p className="text-center text-xs text-muted-foreground mt-8">
        نسخه ۱٫۰٫۰ — تمام اطلاعات فقط روی دستگاه شما ذخیره می‌شوند.
      </p>
    </div>
  );
}
